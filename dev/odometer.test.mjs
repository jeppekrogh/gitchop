...
assert.equal(reels(odo)[0].children.length, 31, 'a blank and three runs: the last reel spins 2.55 s, about twenty digits, two turns at most, and a run to tick into');
...
assert.equal(reels(odo)[0].children[30].textContent, '9');
...
// its right 800 ms after the one before, at about eight digits a second: the first two straight
...
assert.equal(percent(reels(odo)[3]), 'translateY(-54.84%)', 'a cell is a thirty-first of the strip');
...
odo.set(1234);
assert.deepEqual(at(odo), [2, 3, 14, 25], 'new digits, and the last reel a turn more: a four is nearer its pace with two than a six was');
...
assert.equal(reels(odo)[0].children.length, 41, 'four runs for five reels: the fifth spins 3.35 s, about twenty-seven digits, three turns at most');
...
assert.equal(reels(odo)[0].children.length, 21, 'two runs for two reels: the second spins under a second, one turn at most');
...
assert.equal(reels(odo)[0].children.length, 51, 'five runs for seven reels: the seventh spins 4.95 s, about forty digits, four turns at most');
